...
#include <stdlib.h>
#include <string.h>
// Below functions are based from
// https://stackoverflow.com/questions/1577161/passing-a-structure-through-sockets-in-c

inline unsigned char *serialize_int_big_endian(unsigned char *buffer,
                                               int value) {
...
inline unsigned char *serialize_int_little_endian(unsigned char *buffer,
                                                  int value) {
  /* Write little-endian int value into buffer; assumes 32-bit int and 8-bit
   * char. */
  buffer[0] = value;
  buffer[1] = value >> 8;
...
inline unsigned char *deserialize_int_big_endian(unsigned char *buffer,
                                                 int *value) {
  *value = 0;
  for (int i = 0; i < 4; i++) {
...
inline unsigned char *deserialize_int_little_endian(unsigned char *buffer,
                                                    int *value) {
...
inline unsigned char *deserialize_char_array(unsigned char *buffer,
                                             char *value) {
  int i = 0;
  while (buffer[i] != '\0') {
    value[i] = buffer[i];
