...
void *OpenFile(void *frw) {
  Error<void, FileIO> *error =
      new Error<void, FileIO>(&FileIO::Open, AsFileReadWriter(frw));
  error->Execute();
...
void *CloseFile(void *frw) {
  Error<void, FileIO> *error =
      new Error<void, FileIO>(&FileIO::Close, AsFileReadWriter(frw));
  error->Execute();
...
void *WriteToFile(void *frw, int offset, const char *data) {
  Error<int, FileIO, const char *, int> *error =
      new Error<int, FileIO, const char *, int>(&FileIO::WriteToFile,
                                                AsFileReadWriter(frw));
...
void *ReadFromFile(void *frw, char *data, int offset, int numberOfBytesToRead) {
  Error<int, FileIO, char *, int, int> *error =
      new Error<int, FileIO, char *, int, int>(&FileIO::ReadFromFile,
                                               AsFileReadWriter(frw));
