...
private:
  ...
  inline void bindServerSocketAddress() {
    ...
    servaddr.sin_addr.s_addr = htonl(INADDR_ANY); // allow all address to
                                                  // connect
  ...
  inline std::string getDestitionFilePath(std::string destinationPathFile) {
    ...
    filepath = std::string(dirname(&filepath[0])) + "/" +
               std::string(basename(&filepath[0]));
    return filepath;
  }

  void setResponseStatus(ResponseStatus *responseStatus,
                         ResponseStatus_Status status, std::string filepath) {
  ...
  ResponseStatus *handleDestinationFilePathError(ResponseStatus_Status status,
                                                 std::string destFile) {
    if (status ==
        ResponseStatus_Status_FILE_NOT_FOUND) { // parent dir did not exist
  ...
  std::vector<ResponseFile>
  createFilesInDirectory(CreateRequest request, std::string directoryFilePath) {
    ...
    for (int i = 0; i < request.files_size(); i++) {
      ...
      if (!copyResult) {
        status = currentFile.isdir()
                     ? ResponseStatus_Status_FAILED_TO_CREATE_DIRECTORY
                     : ResponseStatus_Status_FAILED_TO_CREATE_FILE;
      }

      std::shared_ptr<ResponseStatus> responseStatus =
          std::make_shared<ResponseStatus>();
      setResponseStatus(responseStatus.get(), status,
                        currentFile.sourcefilepath());
  ...
  renameDestinationFilePath(CreateRequest request,
  ...
  void handleCreateRequest(CreateRequest request) {
    ...
    std::string destinationFilePath =
        getDestitionFilePath(request.destinationfilepath());
    ...
    if (status == ResponseStatus_Status_OK) {
      ...
      responseStatus =
          handleDestinationFilePathError(status, destinationFilePath);
    }
    ...
    Response response(
        connfd, responseFactory.GetSerializedCreateResponse(createResponse));
    response.Write();
  ...
  void handleGetRequest(GetRequest request) {
    ...
    Response response(connfd,
                      responseFactory.GetSerializedGetResponse(getResponse));
    response.Write();
  ...
  void handleReadRequest(ReadRequest request) {
    ...
    Response response(connfd,
                      responseFactory.GetSerializedReadResponse(readReponse));
    response.Write();
  ...
  void handleWriteRequest(WriteRequest request) {
    ...
    Response response(
        connfd, responseFactory.GetSerializedWriteResponse(writeResponse));
    response.Write();
