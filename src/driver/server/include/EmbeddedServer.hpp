...
class EmbeddedServer {
  ...
  private:
	...
	char filepath[MAX_FILEPATH_LENGTH],
		rootFolder[20]; // TODO make 20 constant
	...
	inline void bindServerSocketAddress() {
		...
		servaddr.sin_addr.s_addr = htonl(INADDR_ANY); // allow
													  // all
													  // address
													  // to
													  // connect
	...
	// createAndBindServerSocket creates the server socket and
	// binds the socket
	...
	void setResponseStatus(ResponseStatus *responseStatus, ResponseStatus_Status status,
						   std::string filepath) {
		if (status == ResponseStatus_Status_OK) {
			responseStatus->set_body(response::ResponseStatus_Status_Name<int>(status));
		} else {
			FileIOException e(response::ResponseStatus_Status_Name<int>(status), filepath);
	...
	// TODO rename
	ResponseStatus *handleDestinationFilePathError(ResponseStatus_Status status,
												   std::string destFile) {
		if (status == ResponseStatus_Status_FILE_NOT_FOUND) { // parent dir
															  // did not exist
	...
	std::vector<ResponseFile> createFilesInDirectory(CreateRequest request,
													 std::string directoryFilePath) {
		...
		for (int i = 0; i < request.files_size(); i++) {
			...
			bool copyResult =
				currentFile.isdir()
					...
					: FileIO::CopyFileIntoDirectory(directoryFilePath, currentFile.sourcefilepath(),
													currentFile.filesize());

			ResponseStatus_Status status = ResponseStatus_Status_OK;
			if (!copyResult) {
				status = currentFile.isdir() ? ResponseStatus_Status_FAILED_TO_CREATE_DIRECTORY
	...
	ResponseStatus_Status renameDestinationFilePath(CreateRequest request,
		...
		if (request.issourcedirectory()) {
			...
			std::string newSourcePath =
				std::string(dirname(&destinationFilePath[0])) + "/" + request.sourcefilepath();
			if (!FileIO::RenameFile(destinationFilePath, newSourcePath) &&
				FileIO::CreateFile(newSourcePath.c_str(), request.files(0).filesize())) {
	...
	void handleCreateRequest(CreateRequest request) {
		...
		ResponseStatus_Status status = responseFactory.GetResponseStatusFromFRWStatus(
	...
	void handleGetRequest(GetRequest request) {
		...
		if (status == ResponseStatus_Status_OK) {
			std::vector<frw::File> files = FileIO::GetFilesAtPath(destinationFilePath);

			for (File file : files) {
				getResponse.add_files()->CopyFrom(
					requestFactory.CreateRequestFileFromFRWFile(file));
			}
