...
bool FileServer::HandleClientRequest() {
  ...
  if (bytes ==
      0) { // read nothing from the client then close the connection with it
  ...
  case RequestType::CREATE: {
    handleCreateRequest(
        requestFactory.CreateCreateRequest(request.message.c_str()));
    break;
  }
  ...
  case RequestType::READ: {
    handleReadRequest(
        requestFactory.CreateReadRequest(request.message.c_str()));
    break;
  }
  case RequestType::WRITE: {
    handleWriteRequest(
        requestFactory.CreateWriteRequest(request.message.c_str()));
    break;
  }
