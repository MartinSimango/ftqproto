...
class FileClient {
  ...
  private:
	...
	// connectToServer to the connects to the server specified
	// by serverPort
  ...
  public:
	...
	// specified by the ServerPort returns false if connect
	// failed and errorMessage is set
	...
	// Close closes the connection to the server, returns false
	// upon failure
	void Close();
};
