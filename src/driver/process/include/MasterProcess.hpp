...
private:
  ...
  /**
   * createProcess creates a process object. A process object is an object
   * representation of a currently running process
   *
   *
   * @param processName The name of the process
   * @param processType The type of process to be created i.e WORKER_PROCESS
   * @param cpuId The cpu id the process is to be run on if process affinity is
   * available on OS
   * @return a pointer to the created process
   */
  ftq_domain::Process *createProcess(std::string processName,
                                     ftq_domain::ProcessType processType,
    ...
    switch (processType) {
      ...
      case ftq_domain::ProcessType::WORKER_PROCESS: {
        ...
        WorkerProcess *workerProcess =
            processFactory->createWorkerProcess(processName, cpuId);
  ...
  ftq_domain::Process *spawnProcess(std::string processName,
                                    ftq_domain::ProcessType processType,
                                    ftq_domain::uint8 cpuId) {
  ...
  void startWorkerProcesses(ftq_domain::uint8 n) {
    ftq_domain::Logger::logInfo("Master process starting " + std::to_string(n) +
                                " worker processes");

    for (ftq_domain::uint8 i = 0; i < n; i++) {

      Process *process =
          spawnProcess("Worker process " + std::to_string(i),
...
public:
  MasterProcess(ProcessFactory *processFactory)
      : ftq_domain::Process("Master Process",
                            ftq_domain::ProcessType::MASTER_PROCESS, getpid()) {
